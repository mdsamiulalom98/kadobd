...
class CustomerController extends Controller
{
    ...
    public function order_save(Request $request)
    {
        ...
        $lastOrder = Order::where('invoice_id', 'like', "{$year}{$month}%")->max('invoice_id');

        if ($lastOrder) {
            $lastSerial = (int) substr($lastOrder, -5);
            $newSerial = str_pad($lastSerial + 1, 5, '0', STR_PAD_LEFT);
        } else {
            $newSerial = '00001';
        }

        $invoice_id = "{$year}{$month}{$newSerial}";
    ...
    public function profile_update(Request $request)
    {
        ...
        $update_data->bkash_no     =   $request->bkash_no;
