...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
       $shurjopay = PaymentGateway::where(['status' => 1, 'type' => 'shurjopay'])->first();
        ...
        $brands = Brand::where('status',1)->select('id','name','slug','status','image')->orderBy('name','asc')->get();
        ...
        $pages = CreatePage::where('status',1)->limit(4)->get();
        view()->share('pages',$pages);

        $pagesright = CreatePage::where('status',1)->skip(4)->limit(5)->get();
