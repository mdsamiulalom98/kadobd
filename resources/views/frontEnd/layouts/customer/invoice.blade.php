                    ...
                    <table style="width:100%">
                        <tr>
                            <td style="width: 40%; float: left; padding-top: 15px;">
                                <img src="{{asset($generalsetting->white_logo)}}" width="190px" style="margin-top:20px !important;margin-bottom:10px !important" alt="">
                                <p style="font-size:16px;line-height:1.8;color:#222"><strong>Payment Method:</strong> {{$order->payment?$order->payment->payment_method:''}}</p>
                                @if($order->payment?$order->payment->trx_id:'')
                                <p style="font-size:16px;line-height:1.8;color:#222"><strong>Trx ID:</strong>{{$order->payment?$order->payment->trx_id:''}}</p>
                                <p style="font-size:16px;line-height:1.8;color:#222"><strong>Sender:</strong>{{$order->payment?$order->payment->sender_number:''}}</p>
                                @endif
                            ...
                            <td  style="width:60%;float: left;">
                                ...
                                <div class="invoice-bar" style="background:#fff; transform: skew(36deg); width: 80%; margin-left: 150px; padding: 12px 32px; margin-top: 6px;text-align:right">
                                ...
                                <div class="invoice_to" style="padding-top: 20px;">
                                    ...
                                    <p style="font-size:16px;line-height:1.8;color:#222;text-align: right;font-weight:normal">{{$order->shipping?$order->shipping->district:''}}</p>
                                    <p style="font-size:16px;line-height:1.8;color:#222;text-align: right;font-weight:normal">{{$order->shipping?$order->shipping->upazila:''}}</p>
                                    <p style="font-size:16px;line-height:1.8;color:#222;text-align: right;font-weight:normal">{{$order->shipping?$order->shipping->village_name:''}}</p>
                                    <p style="font-size:16px;line-height:1.8;color:#222;text-align: right;font-weight:normal">{{$order->shipping?$order->shipping->word_no:''}}</p>
                        ...
                        <table class="table" style="width: 300px; float: right;    margin-bottom: 30px;">
                            <tbody style="background:#f1f9f8">
                                <tr>
                                    ...
                                    <td><strong>৳{{$order->amount+$order->discount - ($order->cod_charge)}}</strong></td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td><strong>৳{{$order->cod_charge}}</strong></td>
                                </tr>
                                @endif
                                <tr>
                                ...
                                <tr style="background:#4DBC60;color:#fff">
                                    ...
                                    <td><strong>৳{{($order->amount + $order->discount +$order->shipping_charge) - ($order->cod_charge)}}</strong></td>
                                </tr>
                                 @if($order->savings_pay > 0)
                                <tr style="background:#00aef0;color:#fff">
                                    <td><strong>Savings Pay</strong></td>
                                    <td><strong>৳{{$order->savings_pay}}</strong></td>
                                </tr>
                                <tr style="background:#00aef0;color:#fff">
                                    <td><strong>Due Amount</strong></td>
                                    <td><strong>৳{{($order->amount + $order->discount +$order->shipping_charge) - ($order->cod_charge+$order->savings_pay)}}</strong></td>
                                </tr>
                                @endif
                            </tbody>
                        </table>
